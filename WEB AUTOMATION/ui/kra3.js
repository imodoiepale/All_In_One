...
export async function kraScript() {
  console.log("kraScript function is called!");
  ...
  const updateTable = (id, password, status) => {
    const table = document.getElementById("dataTable");

    // Create a new row
    const row = table.insertRow(1); // Insert at the second row (index 1) to avoid header row
    ...
    cell1.innerHTML = "Client Name"; // You can replace this with actual client name
    cell2.innerHTML = id;
  ...
  const checkPassword = async (id, password, row) => {
    ...
    try {
      ...
      try {
        ...
        updateTable(id, password, status); // Call the updateTable function
      } catch (error) {
        if (error.name === "TimeoutError") {
          status = "Invalid";
          console.log(`Client: ${id}\tPassword: ${password}\tStatus: ${status}`);
          updateTable(id, password, "Invalid"); // Update table with an error status
        } else {
